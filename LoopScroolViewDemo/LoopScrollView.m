...
- (UIColor *)color_currentPageControl
{
    if (!_color_currentPageControl) {
        _color_currentPageControl = [UIColor redColor] ;
    }
...
- (void)addScrollView
{
    ...
    //设置contentSize
    _scrollView.contentSize = CGSizeMake(kImageCount * _scrollView.frame.size.width, _scrollView.frame.size.height) ;
    //设置当前显示的位置为中间图片
    [_scrollView setContentOffset:CGPointMake(_scrollView.frame.size.width, 0) animated:NO];
